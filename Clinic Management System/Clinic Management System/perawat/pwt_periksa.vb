...
Public Class pwt_periksa
    ...
    Private Sub pwt_periksa_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadData()
        LoadDokterData()
        dt_tanggal_pemeriksaan.Enabled = False ' Mengatur DateTimePicker menjadi read-only
    End Sub
    ...
    Private Sub DGV_periksa_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_periksa.CellClick
        If e.RowIndex >= 0 Then
            ...
            If row.Cells("tanggal_pemeriksaan").Value IsNot Nothing AndAlso DateTime.TryParse(row.Cells("tanggal_pemeriksaan").Value.ToString(), Nothing) Then
                dt_tanggal_pemeriksaan.Value = DateTime.Parse(row.Cells("tanggal_pemeriksaan").Value.ToString())
            End If
        End If
    End Sub
    ...
    Private Sub btn_delete_Click(sender As Object, e As EventArgs) Handles btn_delete.Click
        Dim noAntrean As String = txt_no_antrean.Text
        Dim tanggalPemeriksaan As DateTime = dt_tanggal_pemeriksaan.Value ' diasumsikan ada DateTimePicker bernama dt_tanggal_pemeriksaan
        ...
        Using connection As New SqlConnection(connectionString)
            Try
                connection.Open()
                Dim command As New SqlCommand("DELETE FROM pemeriksaan WHERE No_Antrean = @No_Antrean AND CONVERT(Date, Tanggal_Pemeriksaan) = @TanggalPemeriksaan", connection)
                command.Parameters.AddWithValue("@No_Antrean", noAntrean)
                command.Parameters.AddWithValue("@TanggalPemeriksaan", tanggalPemeriksaan.Date) ' Hanya bandingkan tanggalnya, bukan termasuk jam
                command.ExecuteNonQuery()
    ...
    Private Sub txt_cari_TextChanged(sender As Object, e As EventArgs) Handles txt_cari.TextChanged
        Using connection As New SqlConnection(connectionString)
            Try
                connection.Open()
                Dim query As String = "SELECT * FROM pemeriksaan WHERE ID LIKE @cari OR Nama_Pasien LIKE @cari OR Diagnosa LIKE @cari OR Resep LIKE @cari OR Jenis_Kelamin LIKE @cari OR Tanggal_Pemeriksaan LIKE @cari OR Nomor_Telepon LIKE @cari OR Keterangan LIKE @cari OR Alamat LIKE @cari OR Dokter LIKE @cari OR Spesialisasi LIKE @cari OR No_Antrean LIKE @cari OR Id_Perawat_History LIKE @cari OR Nama_Perawat_History LIKE @cari ORDER BY ID DESC"
                Dim command As New SqlCommand(query, connection)
                command.Parameters.AddWithValue("@cari", "%" & txt_cari.Text & "%")
                Dim adapter As New SqlDataAdapter(command)
                Dim dataTable As New DataTable()
                adapter.Fill(dataTable)
                DGV_periksa.DataSource = dataTable
    ...
    Private Sub dt_tanggal_pemeriksaan_ValueChanged(sender As Object, e As EventArgs) Handles dt_tanggal_pemeriksaan.ValueChanged
